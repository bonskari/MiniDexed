...
bool CMiniDexed::Initialize (void)
{
	...
	m_pMIDIDevice = new CMIDIKeyboard(this, m_pConfig, m_pUI);
	if (!m_pMIDIDevice)
	{
		delete m_pUI;
		return false;
	...
	m_pDAWController = new CMiniLab3DawConnection(this, static_cast<CMIDIKeyboard*>(m_pMIDIDevice), m_pConfig, m_pUI);
	if (!m_pDAWController)
	{
		delete m_pLooper;
		delete m_pMIDIDevice;
...
void CMiniDexed::Process (bool bPlugAndPlayUpdated)
{
	...
	if (m_pMIDIDevice)
	{
		m_pMIDIDevice->Process(bPlugAndPlayUpdated);
	}
